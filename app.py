...
if vanhempi_paikalla:
    ...
    # --- OSA 2: Saldojen hallinta ja nollaus (UUSI TOIMINNALLISUUS) ---
    st.write("### 💰 Peliajan käyttäminen ja nollaus")
    
    # Työkalu yksittäisten minuuttien vähentämiseen (pelaamista varten)
    st.write("**Vähennä peliaikaa (kun poika alkaa pelata):**")
    ...
    if st.button("💾 Vähennä minuutit saldosta"):
        ...
        if nykyinen_saldo >= kaytetyt_minuutit:
            ...
            tallenna_data()
            st.success(f"Vähennetty {kaytetyt_minuutit} minuuttia pelaajalta {vahennettava_pelaaja}!")
            st.rerun()
        else:
            st.error(f"Pelaajalla {vahennettava_pelaaja} ei ole tarpeeksi peliaikaa jäljellä! (Saldo: {nykyinen_saldo} min)")

    st.write("")
    # Kokonaisvaltainen nollauspainike korostetussa laatikossa
    st.write("**Koko järjestelmän nollaus:**")
    varmistus = st.checkbox("Vahvistan, että haluan tyhjentää molempien saldot nollaan")
    if st.button("🚨 NOLLAA MOLEMPIEN SALDOT", disabled=not varmistus):
        st.session_state.saldot["Poika 1"] = 0
        st.session_state.saldot["Poika 2"] = 0
        tallenna_data()
        st.success("Saldot nollattu onnistuneesti!")
        st.rerun()

    st.markdown("---")
    
    # --- OSA 3: Tehtävälistan hallinta ---

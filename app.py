...
def saldo_kortti_html(nimi, saldo, vari, tavoite, valittu=False, putki=0):
    ...
    valittu_tyyli = f"box-shadow: 0 0 0 3px {vari}, 0 8px 24px rgba(0,0,0,0.5); transform: translateY(-3px);" if valittu else ""
    checkmark = '<div style="position:absolute;top:10px;right:12px;font-size:1.2rem;">✅</div>' if valittu else ""
    return f"""
    <div class="saldo-kortti" style="--kortti-vari: {vari}; position:relative; {valittu_tyyli}">
        {checkmark}
...
with tab_pojat:
    ...
    # 3. NÄKYMÄ: Pelaajalista ja valitun pelaajan alle aukeavat tehtävät
        ...
        # Luodaan pelaajalaatikot vierekkäin – kortti korostuu valittaessa, nappi on kompakti
        ...
        for i, nimi in enumerate(lapset()):
            ...
            with cols[i]:
                st.markdown(saldo_kortti_html(nimi, saldo, vari, tavoite, valittu=on_valittu, putki=putki), unsafe_allow_html=True)
                if st.button(
                    "✅ Valittu" if on_valittu else "👆 Olen tämä!",
                    key=f"pelaaja_nappi_{nimi}",
                    use_container_width=True,
                    type="primary" if on_valittu else "secondary",
                ):
                    st.session_state.valittu_lapsi = None if on_valittu else nimi
                    st.rerun()

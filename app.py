...
def migraatio_tehtavat():
    """Muuntaa vanhan {nimi: minuutit} -rakenteen uuteen {nimi: {minuutit, kategoria, bonus}} -rakenteeseen."""
...
with tab_pojat:
    ...
    # Tehtävät kategoriaryhmissä — myös bonustehtävät näkyvät merkittynä
    tehtava_valinnat = []
    ...
    if st.button("📨 Lähetä vanhemman hyväksyttäväksi", use_container_width=True):
        if tekija != "Valitse...":
            # Puretaan tehtävän nimi selitteestä
    ...
    if st.session_state.odottavat:
        ...
        for tyo in st.session_state.odottavat:
            ...
            st.info(f"**{tyo['tekija']}**: {tyo['tyo']}{bonus_teksti} (+{tyo['minuutit']} min) — lähetetty {tyo.get('aika','')}")
    ...
    if not st.session_state.vanhempi_kirjautunut:
        ...
        if not st.session_state.odottavat:
            ...
            for tyo_item in list(st.session_state.odottavat):
                ...
                st.warning(f"**{tyo_item['tekija']}** teki työn: *{tyo_item['tyo']}*{bonus_teksti} (+{tyo_item['minuutit']} min) — {tyo_item.get('aika','')}")
                c1, c2 = st.columns(2)
                with c1:
                    if st.button(f"👍 Hyväksy", key=f"hyv_{tyo_item['id']}"):
                        ...
                        # Poistetaan bonustehtävä listalta hyväksynnän jälkeen
        ...
        # Nykyiset tehtävät taulukkona kategorioittain
        ...
        if muokattava_nimi:
            ...
            uudet_min_muokkaus = st.number_input(
                "Minuutit", min_value=5, max_value=300, value=nykyiset["minuutit"], step=5, key="muokkaa_min"
            )
            uusi_kat_muokkaus = st.selectbox(

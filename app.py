...
# Saavutukset: (id, nimi, ikoni, kuvaus, tarkistus-funktio)
SAAVUTUKSET = [
...
/* Tehtäväkortit */
.tehtava-kat { font-size: 0.7rem; color: #64748B; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px; }
.tehtava-nimi { font-size: 1rem; font-weight: 600; color: #E2E8F0; }
.tehtava-min { font-size: 0.85rem; color: #A78BFA; margin-top: 2px; }
...
def laske_putki(nimi):
    """Kuinka monta peräkkäistä päivää lapsella on töitä."""
    paivat = set()
...
def tarkista_saavutukset(nimi):
    """Tarkistaa ja merkitsee uudet saavutukset. Palauttaa listan uusista."""
...
def viikko_data(nimi, viikkoja_taaksepain=0):
    """Palauttaa tietyn viikon ansaitut minuutit ja töiden määrän."""
...
# ══════════════════════════════════════════════════════════════
# VÄLILEHTI 1: LAPSET  – pelimäinen vaiheistettu näkymä
# ══════════════════════════════════════════════════════════════
with tab_pojat:

    # ── VAIHE 3: Vahvistus lähetetty ──────────────────────────
    ...
    # ── VAIHE 2a: Tehtävän valinta ────────────────────────────
    elif st.session_state.valittu_lapsi is not None:
        lapsi_n = st.session_state.valittu_lapsi
        i_n     = lapset().index(lapsi_n) if lapsi_n in lapset() else 0
        vari_n  = LAPSI_VARIT[i_n % len(LAPSI_VARIT)]
        saldo_n = st.session_state.saldot[lapsi_n]
        putki_n = laske_putki(lapsi_n)

        # Pieni saldokortti ylhäällä
        st.markdown(saldo_kortti_html(lapsi_n, saldo_n, vari_n, tavoite), unsafe_allow_html=True)
        if putki_n > 0:
            tuli = "🔥" * min(putki_n, 5)
            st.markdown(f"""
            <div class="putki-box">
                <div class="putki-numero">{tuli}</div>
                <div class="putki-teksti">{putki_n} päivän putki!</div>
            </div>
            """, unsafe_allow_html=True)

        st.markdown(f"<div style='font-size:1.1rem;font-weight:700;color:#E2E8F0;margin:16px 0 8px 0;'>✅ Valitse tehty työ:</div>", unsafe_allow_html=True)

        # Tehtävät kategorioittain painikkeina
        for kat in KATEGORIAT:
            ...
            if not kat_tehtavat:
                continue
            st.markdown(f"<div style='font-size:0.75rem;color:#64748B;text-transform:uppercase;letter-spacing:0.08em;margin:12px 0 6px 0;'>{kat_ikoni} {kat}</div>", unsafe_allow_html=True)
            for tyo_nimi, tyo_tiedot in sorted(kat_tehtavat.items(), key=lambda x: x[1]["minuutit"]):
                ...
                if st.button(nappi_label, key=f"tyo_{tyo_nimi}", use_container_width=True):
                    st.session_state.valittu_tehtava = tyo_nimi
                    st.rerun()

        st.markdown("---")
        if st.button("⬅️ Vaihda pelaajaa", use_container_width=True):
            st.session_state.valittu_lapsi = None
            st.rerun()

    # ── VAIHE 1: Lapsen valinta ───────────────────────────────
    else:
        st.markdown("<div style='text-align:center;font-size:1rem;color:#94A3B8;margin-bottom:16px;'>Kuka olet?</div>", unsafe_allow_html=True)
        ...
        for i, nimi in enumerate(lapset()):
            ...
            with cols[i]:
                ...
                if st.button(f"Olen {nimi} 👋", key=f"valitse_{nimi}", use_container_width=True):
                    st.session_state.valittu_lapsi = nimi
                    st.session_state.valittu_tehtava = None
                    st.rerun()

        # Saavutukset yhteenvetona
        st.markdown("---")

...
tehty_tyo = st.selectbox("Mikä työ tehtiin?", list(st.session_state.tehtavat.keys()))

if st.button("Lähetä vanhemman hyväksyttäväksi"):
    if tekija != "Valitse...":
        pisteet = st.session_state.tehtavat[tehty_tyo]
        ...
        tallenna_data()
        st.success(f"Hienoa {tekija}! Työ lähetetty tarkistettavaksi.")
        st.rerun()
    else:
...
if vanhempi_paikalla:
    ...
    uusi_nimi = st.text_input("Tehtävän nimi")
